# Ideas — the parking lot

Nothing said in a session gets lost: every idea — Johnny's, an agent's, a review finding
too big for "now" — parks here with a date and a one-line why. The weekly review play
sweeps this list; ideas graduate to `plans.md` or get a dated "not now."

Format: `- [ ] YYYY-MM idea — why it might matter (source)`

## Parked
- [ ] 2026-07 Workshop/events system (`events.json` + deposits + waitlist) — the revenue
  engine; biggest gap between what the site *says* and what it *sells* (review finding).
- [ ] 2026-07 Thank-you page after Stripe payment + tie payment→booking — closes the
  funnel dead-end (review finding).
- [ ] 2026-07 Weekly digest pulls GoatCounter numbers once analytics code lands — "who
  came by" in the Monday note (analytics plan).
- [ ] 2026-07 Marblism intake automation once Johnny connects it — drafts → `_inbox/` →
  PR gate (original design, still unwired).
- [ ] 2026-07 Real Videos library page when Johnny shares his YouTube links.
- [ ] 2026-07 Retreat landing pages per season — when dates exist.
- [ ] 2026-07 External ledger-hash anchor (weekly digest could print the tip hash — a
  free off-repo integrity witness).
- [ ] 2026-07 Link-checker + Lighthouse budget in CI (review "Next" items).
- [ ] 2026-07 WordPress .org long-term role decision (archive vs redirect vs audience) —
  no urgency, Johnny's call when ready.

- [ ] 2026-07 Edit brain files inside /admin (Sveltia collection) — friendlier than the
  GitHub editor, but Sveltia may wrap plain .md in front matter; needs a safe test first
  (parked from the Desk build; GitHub Edit links cover it meanwhile).

## Decided "not now" (don't re-pitch without new facts)
- 2026-07 Newsletter — Johnny: no. (decisions.md #4)
- 2026-07 Refund-policy page — Johnny: no published terms. (decisions.md #3)
- 2026-07 Testimonials — only real, permissioned ones, which don't exist yet. (decisions.md #5)
- 2026-07 Podcast — removed; returns only with real episodes. (decisions.md #1)
